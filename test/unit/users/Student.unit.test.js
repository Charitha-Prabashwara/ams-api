...
test('Should not leak memory when repeatedly creating and deleting students', async () => {
  const iterations = 30;
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const tempStudent = await builder.create();
    await new Student().deleteById(tempStudent.id);
  }
